...
def get_word_labels(sentence, feature):
    ...
    for token in doc:
        if feature == 'pos':
            ...
            l = 0 if token.dep_ == 'nsubj' else (1 if token.dep_ == 'obj' else -1)
            ...
            l = 0 if 'nsubj' in subtree_deps else (1 if 'obj' in subtree_deps else -1)
        labels.append(l)
    return labels
